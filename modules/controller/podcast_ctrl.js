...
module.exports = {
  ...
  get_info_admin: (req, res) => {
    bdd.Podcast.findOne().then((podcast) => {
      bdd.Episode.findAll({
        where: { saison: { [Op.ne]: 0 }, episode: { [Op.ne]: 0 } },
        limit: 1,
        ...
        let return_obj = {
          ...
          itunes_subcategory:
            podcast.itunes_subcategory != null
              ? podcast.itunes_subcategory
              : '',
  ...
  edit_pod_img: (req, res) => {
    ...
    if (req.body.image.startsWith('data:image/png;')) {
      pngToJpeg({ quality: 90 })(img_buffer).then((output) => {
        fs.writeFileSync(
          path.join(__dirname, '../../export/img/pod.jpg'),
          output
        );
      ...
      if (ok_slug) {
        bdd.Episode.create({
          ...
          if (!!req.body.transcript_file_raw) {
            let srt_buffer = new Buffer.from(
              req.body.transcript_file_raw.split(/,\s*/)[1],
              'base64'
            );
            fs.writeFileSync(
              path.join(__dirname, '../../export/srt/' + ep.id + '.srt'),
              srt_buffer
            );
          ...
          if (req.body.img !== null) {
            ...
            if (req.body.img.startsWith('data:image/png;')) {
              pngToJpeg({ quality: 90 })(img_buffer).then((output) => {
                fs.writeFileSync(
                  path.join(__dirname, '../../export/img/' + ep.id + '.jpg'),
                  output
                );
        ...
        function continueTraitementAddEp(ep) {
          let audio_buffer = new Buffer.from(
            req.body.enclosure.split(/,\s*/)[1],
            'base64'
          );

          fs.writeFileSync(
            path.join(__dirname, '../../export/audio/' + ep.id + '.mp3'),
            audio_buffer
          );
          ep.duration = convertHMS(
            Math.trunc(getMP3Duration(audio_buffer) / 1000)
          );
          ...
          function laSuite() {
            ...
            function appendPlaylistArray(_cb) {
              ...
              bdd.EpisodePlaylist.findAll({
                where: { PlaylistId: req.body.playlists[i] },
                order: [['place', 'DESC']],
              }).then((last_ep_pl) => {
                let place =
                  last_ep_pl.length === 0 ? 0 : last_ep_pl[0].place + 1;
                let obj = {
                  EpisodeId: ep.id,
            ...
            appendPlaylistArray(() => {
              bdd.EpisodePlaylist.bulkCreate(playlist_array).then(
                (episode_playlists) => {
                  res.send('OK');
                }
              );
  ...
  get_ep_info_admin: (req, res) => {
    bdd.Episode.findAll({
      ...
      include: [
        bdd.Playlist,
        { model: bdd.PersonEpisode, include: bdd.Person },
      ],
    }).then((episode) => {
      let ep = episode[0];
      ...
      bdd.Episode.findOne({ where: { id: req.body.id } }).then((episode) => {
        if (ok_slug || req.body.slug === episode.slug) {
          ...
          if (!!req.body.transcript_file_raw) {
            ...
            fs.writeFileSync(
              path.join(__dirname, '../../export/srt/' + episode.id + '.srt'),
              srt_buffer
            );
            episode.transcript_file =
              '/srt/' + episode.id + '.' + Date.now() + '.srt';
          }

          episode.save().then(() => {
            // Gestion de la liste de publication
            if (new Date(req.body.pub_date) > new Date()) {
              bdd.Planified.findOne({ where: { EpisodeId: episode.id } }).then(
                (plan) => {
                  if (plan !== null) {
  ...
  edit_ep_img: (req, res) => {
    bdd.Episode.findByPk(req.params.id).then((episode) => {
      ...
      if (req.body.image.startsWith('data:image/png;')) {
        pngToJpeg({ quality: 90 })(img_buffer).then((output) => {
          fs.writeFileSync(
            path.join(__dirname, '../../export/img/' + episode.id + '.jpg'),
            output
          );
  ...
  edit_ep_audio: (req, res) => {
    bdd.Episode.findByPk(req.params.id).then((episode) => {
      ...
      fs.writeFileSync(
        path.join(__dirname, '../../export/audio/' + episode.id + '.mp3'),
        audio_buffer
      );
      episode.duration = convertHMS(
        Math.trunc(getMP3Duration(audio_buffer) / 1000)
      );
      ...
      .then((feed) => {
        ...
        bdd.Podcast.findOne().then((podcast) => {
          ...
          podcast.itunes_category =
            feed.itunes.categories !== undefined
              ? feed.itunes.categories[0]
              : '';
                ...
                bdd.Episode.create({
                  ...
                  download(
                    ...
                    download(
                      ...
                      path.join(
                        __dirname,
                        '../../export/img/' + episode.id + '.jpg'
                      )
                    ).then(() => {
                      episode.img = '/img/' + episode.id + '.jpg';
                      episode.save().then(() => {
                        console.log(
                          'Importation : Episode ' + ep.title + ' importé'
                        );
